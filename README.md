...
<p><strong>Auto-generated reports.</strong> An optional toggle on the Autopilot tab (<em>Auto-generate reports</em>) builds a full report per coder in the background as soon as each coding finishes — Coder A's report is downloadable while Coder B is still running. Format (DOCX / PDF / XLSX / HTML) and section selection (quantitative / qualitative; the code legend is always included) are configurable inline, so the usual loop of "load session from history → open the report dialog → click download" is reduced to one click per coder.</p>
<p><strong>Results-tab coder chooser.</strong> Switching to the Results tab after an autopilot run opens a chooser dialog with two clearly-labelled buttons (<code>Coder A — model_a</code> / <code>Coder B — model_b</code>) instead of the empty-state demo prompt. The chosen coder's data populates the regular Results pipeline (qualitative table, code-distribution plot, downloadable report); a banner on the Results tab shows which coder is active and offers a one-click swap to the other one without re-running anything.</p>
</details>
...
<p><strong>Expert mode — N-rater metrics with a significance test.</strong> A toggle at the top of the Testing tab unlocks an extended dialog that handles more than two coders. After picking a metric (Cohen's κ for exactly 2, Krippendorff's α for 2–N, Fleiss' κ for ≥3), the user uploads N reports and gets the metric value, a 95 % bootstrap CI, and a two-sided p-value (H₀: κ=0) computed from the bootstrap distribution. Significance is shown with the conventional star notation (<code>***</code> p&lt;0.001, <code>**</code> p&lt;0.01, <code>*</code> p&lt;0.05, <code>n.s.</code>). Because providers and models are now plentiful, generating three or more independent codings is cheap, and N-rater agreement becomes practically achievable.</p>
</details>
